...
public class Column implements IColumn {
    ...
    protected NavigableSet<IColumn> previousVersions;
    protected ByteBuffer transactionCoordinatorKey;

    // HL: for Eiger-PORT
    protected long client_id;   // the client who did this write txn
    protected long gst;         // the gst this write txn depends on
    protected final long create_time;  // for staleness expt
    ...
    public Column(ByteBuffer name, ByteBuffer value, long timestamp, ByteBuffer transactionCoordinatorKey) {
        this(name, value, timestamp, null, null, null, null, null, transactionCoordinatorKey, -1, -1);
    }
    ...
    public Column(ByteBuffer name, ByteBuffer value, long timestamp, Long lastAccessTime, Long lastAccessTimeOfAPreviousVersion, Long earliestValidTime, Long latestValidTime, NavigableSet<IColumn> previousVersions, ByteBuffer transactionCoordinatorKey, long client_id, long gst) {
        assert name != null;
        assert value != null;
    ...
    @Override
    public void setTransactionCoordinatorKey(ByteBuffer transactionCoordinatorKey) {
        this.transactionCoordinatorKey = transactionCoordinatorKey;
    }
    ...
    private void removeOldPreviousVersions() {
        synchronized (this) {
            ...
            Iterator<IColumn> prevVersionIt = this.previousVersions.descendingIterator();
            IColumn prevVersion;
            Iterator<IColumn> lastIt = null;
            if (prevVersionIt.hasNext()) {
                // we always keep the oldest/default value for safety
                prevVersion = prevVersionIt.next();
                if (this.earliestValidTime() <= safe_time) {
                    this.previousVersions.clear();
                    this.previousVersions.add(prevVersion);
                    return;
                }
            }
            while (prevVersionIt.hasNext()) {
                prevVersion = prevVersionIt.next();
                if (prevVersion.earliestValidTime() <= safe_time) {
                    if (lastIt != null) {
                        lastIt.remove();
                    }
                    lastIt = prevVersionIt;
                } else {
                    return;
                }
            }
    ...
    protected Column updatePreviousVersion(Column previousColumn) {
        ...
        */
        ...
        // HL: we need to keep old versions all the time.
        addPreviousVersion(previousColumn);
    ...
    public IColumn localCopy(ColumnFamilyStore cfs, Allocator allocator) {
        return new Column(cfs.internOrCopy(name, allocator), allocator.clone(value), timestamp, lastAccessTime, lastAccessTimeOfAPreviousVersion, earliestValidTime, latestValidTime, previousVersions, transactionCoordinatorKey, client_id, gst);
    }
    ...
    public long client_id() { return client_id; }

    public long gst() { return gst; }
}
